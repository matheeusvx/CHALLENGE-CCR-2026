...
4. Informe as medidas individuais em `height_measurements_cm`. O pipeline calculará quantidade, mínimo, média, p50, p90 e máximo pelo método linear determinístico.
5. Se houver apenas uma medida, use `measured_height_cm` e `confirmed_single_measurement`. P50 e p90 não serão inventados.
