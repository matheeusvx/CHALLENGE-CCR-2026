...
def test_records_additional_quality_reasons() -> None:
    ...
    assert assessment.quality_reasons == (
        ...
        "insufficient_valid_pixel_count",
        "missing_scl",
...
def test_rejects_scene_below_absolute_pixel_requirement() -> None:
    assessment = assess_scene_quality(
        ...
        partial_raster_coverage=False,
        has_valid_ndvi_pixels=True,
        include_low_quality_scenes=False,
    )
    assert assessment.accepted_for_timeseries is False
    assert assessment.min_pixel_requirement_met is False
    assert "insufficient_valid_pixel_count" in assessment.quality_reasons


@pytest.mark.parametrize(
    ("has_scl", "coverage", "partial", "reason"),
    [
        (False, 100.0, False, "missing_scl"),
        (True, 94.9, False, "insufficient_aoi_coverage"),
        (True, 100.0, True, "partial_raster_coverage"),
    ],
)
def test_rejects_hard_operational_quality_failures(
    has_scl: bool,
    coverage: float,
    partial: bool,
    reason: str,
    ...
    assert assessment.accepted_for_timeseries is False
    assert reason in assessment.quality_reasons


def test_scene_quality_score_is_deterministic_and_bounded() -> None:
    ideal = calculate_scene_quality_score(
        valid_pixel_percentage=100.0,
    ...
    poor = calculate_scene_quality_score(
        valid_pixel_percentage=0.0,
        valid_pixel_count=0,
        min_valid_pixel_count=30,
        aoi_coverage_percentage=0.0,
        has_scl=False,
        scl_class_percentages={},
        cloud_cover=100.0,
    )
...
def test_max_scenes_is_applied_after_quality_assessment() -> None:
    records = [
        {
            "item_id": f"scene-{day:02d}",
            "datetime": f"2026-08-{day:02d}T10:00:00+00:00",
            "accepted_for_timeseries": day != 15,
        }
    ...
    assert "scene-15" not in {record["item_id"] for record in selected}
    assert [record["item_id"] for record in discarded] == ["scene-01", "scene-02"]
